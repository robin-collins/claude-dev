...
const HistoryView: React.FC<HistoryViewProps> = ({ taskHistory, onDone }) => {
                        ...
                        .map((item, index) => (
                            <div
                                ...
                                <div
                                    ...
                                    {(item.excludedFiles || item.whitelistedFiles) && (
                                        <div style={{ marginTop: "8px" }}>
                                            {item.excludedFiles && item.excludedFiles.length > 0 && (
                                                <div>
                                                    <span style={{ fontWeight: 500, color: "var(--vscode-descriptionForeground)" }}>Excluded Files:</span>
                                                    <ul style={{ margin: "4px 0", paddingLeft: "20px", color: "var(--vscode-descriptionForeground)" }}>
                                                        {item.excludedFiles.map((file, index) => (
                                                            <li key={index}>{file}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            )}
                                            {item.whitelistedFiles && item.whitelistedFiles.length > 0 && (
                                                <div>
                                                    <span style={{ fontWeight: 500, color: "var(--vscode-descriptionForeground)" }}>Whitelisted Files:</span>
                                                    <ul style={{ margin: "4px 0", paddingLeft: "20px", color: "var(--vscode-descriptionForeground)" }}>
                                                        {item.whitelistedFiles.map((file, index) => (
                                                            <li key={index}>{file}</li>
                                                        ))}
                                                    </ul>
                                                </div>
